...
for i in range(128):
    for j in range(128):
        ...
        log_pdf, elapsed, N, D = adapt_exposure(prior, tfront[i,j], ent_threshold, Ms, dead_time = 810, mode = 0)
        d = np.argmax(log_pdf)
        if d > 490 or d < 10:
            ad_img[i, j] = d
        else:
        ...
        log_pdf, elapsed, N, D = adapt_exposure(prior, tfront[i,j], ent_threshold, Ms, dead_time = 810, mode = 1)
        d = np.argmax(log_pdf)
        if d > 490 or d < 10:
            frm_img[i, j] = d
        else:
